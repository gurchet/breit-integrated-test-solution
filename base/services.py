...
def setup_appium():
    ...
    if CurrentRun.current_appium is None:
        ...
        if is_appium_running_on_port(CurrentRun.current_appium.port):
            ...
            start_appium_service(CurrentRun.current_appium.port)
            print("Appium server running")
            return True
...
def setup_device():
    ...
    if Args.get('app_path') is not None:
        if install_app_on_device(CurrentRun.current_device.get_name(), Args.get('app_path')) != 0:
...
if __name__ == '__main__':
    print(get_config("appium_auto_run"))
    Args.set_arguments()
